...
{
	float input[N] = { 0 };
	float input_len = sizeof(input);
	int i;

	printf("******请输入%d个浮点数数以“ ”作为分割符******\n", N);
	for (i = 0; i < N; i++)
	{
		scanf_s("%f", &input[i]);
	}
	...
	// 执行锁内代码输出计算后的结果
	ret = slm_execute_static(slm_handle, "h5first2.evx", (SS_BYTE*)input, input_len, output, MAX_BUFFER_SIZE, &ret_len);
	if (ret != 0)
	{
