...
*   slm_keep_alive 是登录后必须要调用，让 Virbox许可服务 知道APP应用还还活着，没有死机或者被调试。
*   
*   使用许可调用顺序：
...
typedef struct _ST_LOCK_INFO {
    SS_UINT32           size;               //  结构体大小
    SS_UINT8            device_type;        //  设备类型，0 = 普通用户锁；2 = 时钟用户锁
    SS_UINT8            device_mode;        //  设备型号, 0 = 精锐5用户锁
    SS_UINT16           res;                //  保留
    SS_UINT8            lmfirm_version[4];  //  Entry固件版本
    SS_UINT32           h5firm_version;     //  H5固件版本
    SS_UINT32           hardware_version;   //  硬件版本
    SS_UINT32           manufacture_date;   //  生产日期
    SS_UINT32           total_space;        //  总共空间
    SS_UINT32           available_space;    //  剩余空间
    SS_UINT32           protocol;           //  通讯协议
    SS_UINT32           slave_addr;         //  总线地址
    SS_UINT32           clock;              //  锁内时钟（UTC）
    SS_UINT32           clock_type;         //  0 = 虚拟时钟；2 = 硬件时钟
    SS_UINT32           drop_times;         //  时钟锁掉电次数
    SS_BYTE             lock_sn[SLM_LOCK_SN_LENGTH];    //  锁号
    SS_BYTE             user_info[128];     //  锁的编码区：用户码区，存储了外壳编号
    SS_BYTE             inner_info[128];    //  锁的编码区：内部码区
} ST_LOCK_INFO;

typedef struct _ST_SESSION_INFO {
    SS_UINT32           size;                     //  结果大小
    SS_UINT32           license_id;               //  已经登录的许可id 
    union
    {
        SS_BYTE         sn[SLM_LOCK_SN_LENGTH];
        SS_BYTE         user_guid[SLM_GUID_LENGTH];
    };
    SS_UINT32           app_time_out;          //  开发商设置的time_out
    SS_UINT32           app_process_id;        //  进程id
    SS_UINT8            mac[SLM_MAC_SIZE];        //  网卡MAC
    char                session_id[SLM_MAX_SESSION_ID_SIZE];
    char                cpuinfo[SLM_MAX_CPU_INFO_SIZE];   //  CPU ID Verndor 信息
    char                bios[SLM_MAX_BIOS_INFO_SIZE];     //  BIOS信息 主板名称等   
} ST_SESSION_INFO;

typedef struct _ST_SLM_LICENSE_INFO{
    SS_UINT32   size;
    SS_UINT32   lic_ver;        //  许可版本号
    SS_BYTE     flag[4];        //  许可标志位（注：不要改动flag的偏移）
    SS_BYTE     guid[SLM_LOCK_SN_LENGTH];       //  许可的guid
    SS_UINT32   lic_id;         //  许可ID
    SS_UINT32   start_time;     //  起始时间
    SS_UINT32   end_time;       //  终止时间
    SS_UINT32   span_time;      //  时间跨度
    SS_UINT32   time_stamp;     //  升级时间戳
    SS_UINT16   serial;         //  升级流水号
    SS_UINT16   concurrency;    //  许可最大并发数
    SS_UINT32   count;          //  最大次数限制
    SS_UINT64   module_bit;     //  模块功能标识位
    SS_UINT32   lock_time;      //  锁定时间
    SS_UINT32   first_use_time; //  第一次使用时间
    SS_UINT32   curr_count;     //  当前剩余次数
    SS_UINT32   offline_period; //  可离线时间
    SS_UINT32   bind_node;      //  最多可同时使用的机器数量，与机器无关，只与数量有关
    SS_UINT32   max_bind_node;  //  最多可绑定的机总器数，与机器有关，一旦绑定，永久相关
    SS_UINT32   raw_size;   //  读写区大小
    SS_UINT32   rom_size;   //  只读区大小
    SS_UINT32   pub_size;   //  公开区大小
} ST_SLM_LICENSE_INFO;

typedef struct _ST_FILE_INFO {
    SS_BYTE     validate;                /** 标志哪些域有效，仅在设置文件属性中使用              */
    SS_BYTE     type;                    /** 文件类型，不能修改                                 */
    SS_UINT16   pri;                     /** 文件访问权限                                       */
    SS_UINT32   size;                    /** 文件大小                                           */
    SS_UINT32   time;                    /** 文件修改时间                                       */
    SS_BYTE     name[16];                /** 文件名  当前文件名称长度16字节                     */
} ST_FILE_INFO;
...
typedef struct _ST_DEV_INFO {
    ...
    SS_UINT16           firm_type;      //  LM类型（LM_FIRM_TYPE_XXX)
...
/** 
*   @defgroup group1_lm_runtime  Virbox Runtime Virbox用户运行时库
*   This is Virbox lm_runtime API Virbox用户运行时库
*   @{
*/
...
*   @remarks    slm_init函数是 Virbox Runtime API 的初始化函数，在使用其他 Virbox Runtime API之前，必须要先调用此函数，
...
*   @param[in]  format          参数格式，参考INFO_FORMAT_TYPE，目前仅支持JSON和STRUCT
...
*               备注：有关0号许可的介绍，可参考《Virbox许可体系与许可管理》文档中关于0号许可的描述
...
*   @param[in]  format      信息格式，参考 INFO_FORMAT_TYPE，当前仅支持JSON和STRUCT
...
*   @return     成功返回SS_OK，失败返回相应的错误码
*   @remarks    Virbox许可服务 的托管内存原理是 APP利用有效的许可作为凭证，在 Virbox许可服务 模块内数据加密且数据校验，其内存二进制数据没有明文，
*               并且无法非法修改。黑客极难查看与篡改使用。
*
*               用户可以把自己APP的一些敏感数据保存到 Virbox许可服务 的托管内存，比如帐号口令，SQL数据库的帐号与密码，涉及到操作权限的临时数据放到 Virbox许可服务 内存托管里面。
*               另外一方面APP跟 Virbox许可服务 耦合度极大的提高，防止黑客脱离 Virbox许可服务 调试与运行。
*
*               内存托管的好处：
*               -# 敏感数据内存不泄密、无法篡改。
*               -# 可以跨线程安全交互数据。
...
/*!
*   @brief      获得 Runtime API 和 Virbox许可服务 的版本信息.
...
*               第四步、通过调用slm_d2c_update_inside 接口，将锁内生成的中间文件转化为正式的 Virbox 许可。
*
